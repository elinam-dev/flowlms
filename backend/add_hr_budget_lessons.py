#!/usr/bin/env python3
"""
Rename hr_budget images, add as lessons to
Mastering HR Budget: Strategic financial planning for professionals and publish
"""
import asyncio
...
async def add_lessons():
    ...
    course = await db.courses.find_one(
        {"title": {"$regex": "mastering hr budget", "$options": "i"}},
        {"_id": 0}
    )
    if not course:
    ...
    for i, filename in enumerate(renamed):
        url = f"{BASE_URL}/uploads/images/HR/hr_budget/{filename}"
