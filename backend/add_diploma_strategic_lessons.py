#!/usr/bin/env python3
"""
Add all diploma_in_strategic images as lessons to Diploma in strategic HR
"""
import asyncio
...
IMG_DIR = ROOT_DIR / "uploads" / "images" / "HR" / "diploma_in_strategic"

async def add_lessons():
    files = sorted([f.name for f in IMG_DIR.iterdir() if f.suffix.lower() == '.jpeg'])
    print(f"Found {len(files)} images")

    course = await db.courses.find_one(
        {"title": {"$regex": "diploma in strategic hr", "$options": "i"}},
        {"_id": 0}
    )
    if not course:
    ...
    for i, filename in enumerate(files):
        url = f"{BASE_URL}/uploads/images/HR/diploma_in_strategic/{filename}"
    ...
    print(f"Added {len(files)} lessons and published the course.")
