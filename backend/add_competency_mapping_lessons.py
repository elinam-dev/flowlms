#!/usr/bin/env python3
"""
Rename competency_mapping images, add as lessons to
Competency Mapping in HR: Transform people into Human capital and publish
"""
import asyncio
...
IMG_DIR = ROOT_DIR / "uploads" / "images" / "HR" / "competency_mapping"
...
async def add_lessons():
    ...
    for i, f in enumerate(files, 1):
        new_name = f"competency_mapping_{i:03d}.jpeg"
    ...
    course = await db.courses.find_one(
        {"title": {"$regex": "competency mapping", "$options": "i"}},
        {"_id": 0}
    )
    if not course:
    ...
    for i, filename in enumerate(renamed):
        url = f"{BASE_URL}/uploads/images/HR/competency_mapping/{filename}"
