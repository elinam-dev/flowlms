#!/usr/bin/env python3
"""
Rename respect_foundations images and add as lessons to
...
IMG_DIR = ROOT_DIR / "uploads" / "images" / "HR" / "respect_foundations"
...
async def add_lessons():
    ...
    for i, f in enumerate(files, 1):
        new_name = f"respect_foundations_{i:03d}.jpeg"
    ...
    course = await db.courses.find_one(
        {"title": {"$regex": "respect foundations", "$options": "i"}},
        {"_id": 0}
    )
    if not course:
    ...
    for i, filename in enumerate(renamed):
        url = f"{BASE_URL}/uploads/images/HR/respect_foundations/{filename}"
        ...
        print(f"  [{i+1}] Slide {i+1}")

    print(f"\nAdded {len(renamed)} lesson(s). Course NOT published yet — more content to come.")
