...
IMG_DIR = ROOT_DIR / "uploads" / "images" / "HR" / "behavioral_interviewing"

async def add_lessons():
    files = sorted([f for f in IMG_DIR.iterdir() if f.suffix.lower() in ['.jpeg', '.jpg']])
    renamed = []
    for i, f in enumerate(files, 1):
        new_name = f"behavioral_interviewing_{i:03d}.jpeg"
    ...
    course = await db.courses.find_one({"title": {"$regex": "behavioral interviewing", "$options": "i"}}, {"_id": 0})
    if not course:
    ...
    for i, filename in enumerate(renamed):
        url = f"{BASE_URL}/uploads/images/HR/behavioral_interviewing/{filename}"
